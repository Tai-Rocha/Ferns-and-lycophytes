...
bonita_gavea <- read.csv("./data/Final_records/Bonita_Gavea.csv", sep = ",", dec = ".")
...
plot(predic)
######
sp <- unique(bonita_gavea$sp)

setup_sdmdata(species_name = sp,
              occurrences = bonita_gavea[,-1], 
              predictors = predic, 
              lon = "longitude", 
              lat = "latitude", 
              models_dir = "./results/Final_Cleanning/thinning_bonita_gavea", 
              real_absences = NULL, 
              buffer_type = NULL, 
              dist_buf = NULL, 
              env_filter = FALSE, 
