...
PCA_INPUT <- read.csv("./data/PCA/PCA_INPUT.csv", sep = ",", dec = ".")
...
res.pca <- prcomp(PCA_INPUT[1:23],  scale = TRUE)



# Color individuals by groups
fviz_pca_ind(res.pca, label="none", habillage=PCA_INPUT$Setor)


p <- fviz_pca_ind(res.pca, label="none", habillage=PCA_INPUT$Setor,
                  addEllipses=TRUE, ellipse.level=0.95)
print(p)

# Change the color by groups, add ellipses
fviz_pca_biplot(res.pca, label="var", habillage=PCA_INPUT$Setor,
                addEllipses=TRUE, ellipse.level=0.95)
...
fviz_pca_var(res.pca, select.var = list(contrib = 5))
